...
function FindReg(){
    ...
    const findRegister=()=>{
      alert("Searching")
      Axios.post("http://localhost:3001/findByName",{firstname:firstname,lastname:lastname,fathername:fathername,mothername:mothername,date:date,gender:gender,email:email,password:password,phoneno:phoneno,}).then((res)=>{setAddress(res.data.address); console.log(res.data.address)})
    }
      return(
    <div>
    <h1 align="center">Searching Register</h1>
    <table border="1" align="center" style={{backgroundColor:"white",border:'solid'}}>
    <tr><label>First name</label><td><input type="text"  onChange={(event)=>{setFirstname(event.target.value)}} /></td></tr>
    ...
    <button onClick={findRegister}>Find Register</button>
    </table>
    <br/>
     Name:{firstname}<br/>
     Address:{address}
